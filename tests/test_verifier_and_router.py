...
@pytest.mark.asyncio
async def test_evidence_scoring_strong():
    mock_page = AsyncMock()
    ...
    res = await verify_post_submission_evidence(mock_page, application_id="REQ-99481")
    assert res.status == "CONFIRMED_APPLIED"
    assert res.score == "STRONG"
    assert res.evidence_details["dom_confirmation"] is True
    assert res.evidence_details["application_id"] == "REQ-99481"
...
async def test_evidence_scoring_weak_form_filled():
    ...
    res = await verify_post_submission_evidence(mock_page)
